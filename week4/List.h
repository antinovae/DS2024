...
private:
  ...
  struct Node {
    ...
    /**
     * @brief 移动构造函数. 唯一的区别是节点数据是一个右值引用. 因此以 std::move
     * 的方式传递.
     *
     * @param d 节点内存放的数据.
     * 只有这里是一个显式的右值引用，才会调用这个构造函数.
     * @param p 前一个节点的指针. 默认为空.
     * @param n 后一个节点的指针. 默认为空.
     */
...
public:
  ...
  public:
    ...
    /// 这里我们看到前自增和后自增的区别. 就是 std::cout << *iter++ <<
    /// std::endl; 和 std::cout << *++iter << std::endl; 的区别.
    /// 前者是先输出当前值，再自增；后者是先自增，再输出.
  ...
  protected:
    ...
    friend class List<Object>; /**<! 使 List 类可以访问到迭代器的私有成员和
                                  protected 成员. */
  ...
  public:
     ...
     * 因为它继承自 const_iterator，而 const_iterator 已经初始化了 current 为
     * nullptr. 子类的构造函数会调用父类的构造函数，所以这里不需要再初始化
     * current 了.
     *
     */
    iterator() {}
    ...
    /**
     * @brief 返回当前节点的数据. 注意这里是可读可写的. 因为它直接调用了父类的
     * retrieve 函数. 而父类的 retrieve
     * 函数返回的是一个引用，并没有限制不能修改. 而父类的 retrieve 函数后缀的
     * const 只限制了它不能修改父类的成员变量. 因此这里所有的规则都是可以遵守的.
     *
     ...
     * 但通过 return 的 const 修饰符来区分是一个例外.
     * 这使得我们可以给同一个函数分别提供动态和静态的版本，
     * 然后根据它们的实际使用情况来选择调用哪一个.
     * 注意这里直接调用的父类的对应静态运算.
     * 严格说我们也可以直接调用父类的运算符, 但这样会使代码更加清晰.
     *
     * @return const Object&
     */
  ...
  protected:
    ...
    friend class List<Object>; /**<! 同样使 List 类可以访问到迭代器的私有成员和
                                  protected 成员. */
  };
...
public:
  ...
  List(const List &rhs) {
    ...
    /// 而这里的 for
    /// 循环的方式实际上让我们可以遍历一个容器，而不需要知道容器的具体类型.
    /// 也就是说，实际上这里是通过 iterator 来遍历的. 这里的 rhs 是一个
    /// List，所以它有 begin 和 end 函数. 这段代码的实际效果等价于
   ...
   * 这个技术使得整个过程逻辑简单，并且效率更高.
   * 但是一些实现的关键技术细节都是在 C++ 14 起引入的，因此只实现了 C++ 11
   * 的版本的课本并没有做到.
   */
  ...
  /// 这里调用静态还是动态，实际上取决于调用的环境. 如果调用的是 const
  /// 对象，那么就会调用 const 的版本.
  ...
  /**
   * @brief 删除指定位置的数据节点.
   * 由于哨兵节点的存在，不用担心删完了以后最后变成一个 nullptr.
   *
   * @param itr 指定位置的迭代器.
