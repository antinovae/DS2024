...
// 双旋转：先右旋再左旋
BinaryNode *doubleWithRightChild(BinaryNode *k1) {
...
BinaryNode *getMin(BinaryNode *&t) {
    if (t->left == nullptr) {
        ...
        t = t->right;  // 通过指针移动删除最小节点
        return minNode;
    }
    return getMin(t->left);  // 查找左子树
}
...
BinaryNode *remove(const Comparable &x, BinaryNode *t) {
    ...
    if (x < t->element) {
        t->left = remove(x, t->left);  // 在左子树中删除
    } else if (x > t->element) {
        t->right = remove(x, t->right);  // 在右子树中删除
    } else {  // 找到要删除的节点
        if (t->left != nullptr && t->right != nullptr) {  // 有两个子节点
            BinaryNode *mn = getMin(t->right);  // 找到右子树中最小的节点
            mn->left=t->left;
            mn->right=t->right;
            delete t;
            t=mn;
            ...
            BinaryNode *oldNode = t;
            t = (t->left != nullptr) ? t->left : t->right;  // 修改指针
            delete oldNode;
        }
    }
...
/*
    BinaryNode * get_min(BinaryNode * &t){
        //函数get_min会返回当前子树中的最小节点
        if(t==nullptr){//t不应该是空的
            assert(0);
        }
        if(t->left!=nullptr){
            auto mn=get_min(t->left);
            if(mn==t->left){//把节点的指针断掉
                t->left=nullptr;
            }
            return mn;//如果有左子树，查找左子树
        }
        else {
            return t;//否则当前节点就是最小的
        }
    }
    BinaryNode * remove(const Comparable &x, BinaryNode * t) {
        //由于我们要做结点替换，所以这个函数会返回当前的结点指针。
        if (t == nullptr) {
            return nullptr;  /// 元素不存在
        }
        if (x < t->element) {
            t->left=remove(x, t->left);//更新指针。
            return balance(t);
        } else if (x > t->element) {
