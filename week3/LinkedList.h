...
template <typename T> T SingleLinkedList<T>::getCurrentVal() const {
  return *currentPos;
}
template <typename T> void SingleLinkedList<T>::setCurrentVal(const T &_val) {
  *currentPos = _val;
}
template <typename T> bool SingleLinkedList<T>::isEmplty() const {
  return size == 0;
}
template <typename T> void SingleLinkedList<T>::insert(T _val) {
  Node *newNode = new Node(_val);
  newNode->next = currentPos->next;
  currentPos->next = newNode;
  currentPos = currentPos->next;
}
template <typename T> void SingleLinkedList<T>::remove() {
  Node *nxt = currentPos->next->next;
  delete currentPos->next;
  currentPos->next = nxt;
}

template <typename T> bool SingleLinkedList<T>::find(const T &_val) {
  Node *p = head;
  while (p != nullptr) {
    if (p->data == _val) {
      currentPos = p;
...
template <typename T> int SingleLinkedList<T>::getSize() const { return size; }
...
template <typename T> void SingleLinkedList<T>::_emptyList() {
  Node *p = head;
  while (p != nullptr) {
    Node *t = p;
    p = p->next;
...
template <typename T> void SingleLinkedList<T>::printList() const {
  Node *p = head;
  while (p != nullptr) {
    std::cout << p->data << "\t";
    p = p->next;
...
template <typename T> void SingleLinkedList<T>::emptyList() {
  _emptyList();
  head = nullptr;
  currentPos = nullptr;
  size = 0;
